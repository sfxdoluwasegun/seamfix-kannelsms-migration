...
public class SMSManager {
	...
	public void sendWAPPush(String host, String port, String receiver, String type, String message, String url, int expirationDays, String dlrmask, String dlrurl) throws WAPPushRequestException, Exception {
		...
		if(Utils.checkNonEmptyStringAttribute(dlrmask))
			connection.setRequestProperty("X-Kannel-DLR-Mask", dlrmask);
		
		if(Utils.checkNonEmptyStringAttribute(dlrurl))
			connection.setRequestProperty("X-Kannel-DLR-URL", dlrurl);
		...
		if (responseCode == null || !responseCode.equals(WAP_PUSH_RESPONSE_SUCCESS_CODE))
			throw new WAPPushRequestException(responseCode);
	...
	private String createSIMessage(String url, String creationDate, String expirationDate, String indicationMessage) {
		String siMessage = "<?xml version=\"1.0\"?>" +
				...
				while(it.hasNext()) {
					...
					synchronized(availableMessages) {
						for(int i=0; i<reqsCount; i++)
							messagesToSend.add(availableMessages.remove(0));
					}
					...
					while(it1.hasNext()) {
						...
						try {
							...
							if(message instanceof SMSMessage)
								...
								sendWAPPushMessage((WAPPushMessage)message);
